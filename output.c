...
void ball_output(void *ball, FILE *output_file) {
     ...
     *((float*)(ball+int_size)), *((int*)ball), ball_surface_area(ball));
}
...
void parallelepiped_output(void *parall, FILE *output_file) {
     ...
     *((float*)(parall+3*int_size)), *((int*)parall), *((int*)(parall+int_size)), *((int*)(parall+2*int_size)), parallelepiped_surface_area(parall));
}
...
void tetrahedron_output(void *tetr, FILE *output_file) {
     ...
     *((float*)(tetr+int_size)), *((int*)tetr), tetrahedron_surface_area(tetr));
}

void figure_output(void *figure, FILE *output_file) {
    int k = *((int*)figure);
    if (k == BALL) {
        ball_output(figure+int_size, output_file);
    }
    else if (k == TETRAHEDRON) {
        tetrahedron_output(figure+int_size, output_file);
    }
    else if(k == PARALLELEPIPED) {
        parallelepiped_output(figure+int_size, output_file);
    }
    else {
